...
class VoiceClient {
    ...
    getServerUrl() {
        ...
        // If empty, use current origin (for Vercel deployment)
        return inputUrl || window.location.origin;
    }
    ...
    async connect() {
        ...
        try {
            ...
            // For Vercel serverless, WebSocket is not available
            // Show configuration info instead
            if (data.azure_config) {
                this.updateStatus('connected', '🟢 Session configured');
                this.addTranscript('system', `Session created: ${data.session_id}`);
                this.addTranscript('system', `Language: ${data.language}`);
                this.addTranscript('system', `Agent: ${data.agent_type}`);
                this.addTranscript('system', 'Note: WebSocket streaming requires a persistent connection server. This serverless deployment provides session configuration for client-side Azure OpenAI connection.');

                this.connectBtn.disabled = true;
                this.disconnectBtn.disabled = false;
                this.isConnected = true;

                // Store config for potential direct Azure connection
                this.sessionConfig = data;
                return;
            }

            // Legacy WebSocket connection (for non-Vercel deployments)
    ...
    addTranscript(role, text) {
        ...
        labelEl.textContent = role === 'user' ? 'You' : (role === 'system' ? 'System' : 'Jean-Guy');
